			   ...
			   const struct i2c_device_id *id)
{
	...
	if(client->dev.of_node){
		platform_data.irq_gpio = of_get_named_gpio(client->dev.of_node, "brcm,gpio_irq", 0);
		platform_data.en_gpio = of_get_named_gpio(client->dev.of_node, "brcm,gpio_ven", 0);
		platform_data.wake_gpio = of_get_named_gpio(client->dev.of_node, "brcm,gpio_wake", 0);
	}
	else{
...
static const struct of_device_id of_bcm2079x_i2c_match[] __maybe_unused = {
	{ .compatible = "brcm,bcm2079x", },
	{},
};
MODULE_DEVICE_TABLE(of, of_bcm2079x_i2c_match);

static const struct i2c_device_id bcm2079x_i2c_id_table[] = {
	{ BCM2079x_I2C_DRIVER_NAME, 0 },
	{}
};
MODULE_DEVICE_TABLE(i2c, bcm2079x_i2c_id_table);

static struct i2c_driver bcm2079x_driver = {
	.id_table = bcm2079x_i2c_id_table,
